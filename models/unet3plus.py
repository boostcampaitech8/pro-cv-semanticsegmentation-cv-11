...
# -----------------------------
# UNet 3+ with deep supervision and class-guided module (CGM)
# -----------------------------
class UNet_3Plus_DeepSup_CGM(UNet_3Plus_DeepSup):
    """
    UNet 3+ DeepSup + CGM (class-guided module).
    This follows the official structure: a classification branch from hd5 gates each segmentation output.
    """
    def __init__(
        self,
        in_channels: int = 3,
        ...
        # Official uses 2 outputs for cls branch (background/foreground), then argmax.
        # Kept as-is conceptually, but implemented without numpy dependency.
        filters = [64, 128, 256, 512, 1024]
        ...
        # init for cls layers too
        for m in self.cls.modules():
            if isinstance(m, nn.Conv2d):
                init_weights(m, init_type="kaiming")

    @staticmethod
    def dotProduct(seg: torch.Tensor, cls: torch.Tensor) -> torch.Tensor:
        # seg: (B, N, H, W), cls: (B, 1) or (B, N)
        B, N, H, W = seg.size()
    ...
    def forward(self, inputs: torch.Tensor):
        # We replicate parent's forward but insert cls gating (kept close to official logic).
        # Encoder
        h1 = self.conv1(inputs)
        h2 = self.conv2(self.maxpool1(h1))
        h3 = self.conv3(self.maxpool2(h2))
        ...
        # Classification branch (B,2,1,1) -> (B,2)
        cls_branch = self.cls(hd5).squeeze(3).squeeze(2)
        cls_branch_max = cls_branch.argmax(dim=1)          # (B,)
        cls_branch_max = cls_branch_max[:, None].float()   # (B,1)
        ...
        # CGM gating (official dotProduct)
