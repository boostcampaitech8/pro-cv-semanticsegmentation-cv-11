...
class WristCropDataset(Dataset):
    ...
    def __init__(
        ...
        # 손목 뼈 인덱스 계산
        from utils.crop import get_wrist_indices
        self.wrist_indices = get_wrist_indices(self.class2ind)

        # Fold 분할 (XRayDataset과 동일한 로직)
        if fold == None:
            self.fnames = fnames
            self.labels = labels
        else:
    ...
    def __getitem__(self, item):
        ...
        # === 손목 ROI crop 적용 ===
        from utils.crop import crop_wrist_roi
        image, label = crop_wrist_roi(
            image,
            label,
            wrist_indices=self.wrist_indices,
