...
def build_optimizer(cfg, model):
    ...
    if opt_name == "adam":
        ...
        return optimizer.AdamP(model.parameters(), lr=LR, weight_decay=wd, betas=(0.9, 0.999))
...
def validate(model, val_loader, criterion, cfg):
    ...
    for images, masks in val_loader:
        ...
        masks = masks.cuda(non_blocking=True)   # (B, C, H, W) one-hot
        ...
        #     else:
        #         masks_ce = masks
        #     loss = criterion(logits, masks_ce.long())
        ...
        # Soft Dice 계산
        # --------------------
        # 기존: probs = torch.sigmoid(logits)
        task_type = cfg["model"].get("task_type", "multiclass").lower()
        if task_type == "multiclass":
            probs = torch.softmax(logits, dim=1)
        else:
            probs = torch.sigmoid(logits)

        dice_batch = multiclass_soft_dice_coef(probs, masks, num_classes)

        # utils.py에 구현한 함수 사용
        dice_batch = multiclass_soft_dice_coef(
            probs, masks, num_classes
        )  # (C,)
    ...
    # background 제외 평균 Dice
    mean_dice = dices_per_class[1:].mean()

    # --------------------
    # 출력
    # --------------------
    print("CLASSES index check:")
    for i, c in enumerate(CLASSES[:5]):
        print(i, c)
    print("\n[Validation | Soft Dice per class]")
    print("\n".join(
        f"{c:<12}: {d.item():.4f}"
        for c, d in zip(CLASSES, dices_per_class)
    ))
    print(f"\n[Validation] Mean Soft Dice (w/o bg): {mean_dice.item():.4f}")

    return mean_dice.item(), total_loss / len(val_loader)
...
def main():
    ...
    print("Dataset classes (first 5):", train_ds.classes[:5])
    val_ds = XRayDataset(
        image_root=IMAGE_ROOT,
        label_root=LABEL_ROOT,
        classes=CLASSES,
