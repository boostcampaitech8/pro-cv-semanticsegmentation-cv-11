...
def multiclass_soft_dice_coef(probs: torch.Tensor, targets: torch.Tensor, num_classes: int, eps=1e-6):
    """
    probs   : (B, C, H, W) sigmoid output
    targets : (B, C, H, W) one-hot GT
    """
    dices = []
    ...
    for c in range(num_classes):
        ...
        dice = (2.0 * inter + eps) / (union + eps)  # (B,)
        dices.append(dice.mean())
